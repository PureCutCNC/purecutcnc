...
| P5 — Store slice extraction | 🟡 In progress (`2f308b4`) | merged (template slice) | **First/template slice landed:** `createFeatureSlice` extracted from `projectStore.ts` (7040→~5900) into `src/store/slices/featureSlice.ts` (1141 lines), deps-injection `(set, get, deps)` matching existing slices; `ProjectStore` interface in `types.ts` **unchanged** (frozen). Build green (all 46 test files incl. store tests pass); lint clean. Browser-regressed feature CRUD (add/delete folder, undo, Badge example parse w/ derived Join/Offset features) — 0 console errors. **Remaining:** other domain slices (operations, tools, clamps, tabs, import/merge, backdrop, machine, pending\*, constraints, history), `profileEdit.ts` helper extraction (+tests), then the `src/store/**` max-lines guard once `projectStore.ts` < 600. |
