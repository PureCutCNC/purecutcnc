...
- `src/engine/toolpaths/pocket.ts` - add the offset traversal mode, implement inner-first recursion, and pass it from pocket rough offset generation.
- `src/engine/toolpaths/toolpaths.test.ts` - add focused coverage that a rectangular pocket offset roughing pass emits inner loops before outer wall-adjacent loops and preserves climb/conventional winding.
