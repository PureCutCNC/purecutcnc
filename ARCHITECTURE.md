...
- **Nest record** (`project.nests`, optional, issue #741): disposable sheet-nesting output. Membership lives on the record — `copyIds` (instances the nest minted), `movedOriginals` (pre-nest transforms), `settings` — not as a field on instances, so it survives folder moves and is never copied onto a pasted duplicate. Copies are linked instances of the part's definitions; `discardNest` deletes exactly `copyIds`, strips them from every operation target, and restores `movedOriginals`, as one undo step. Malformed records are dropped on load rather than failing it.
