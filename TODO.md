...
- Status: Open
- Priority: High
- Summary: Surface cleaning operation cuts into protected "island" features when they divide or cross the target feature boundary.
- Notes:
  - The bug occurs because the toolpath is generated by expanding the already-clipped geometry, treating subtracted edges as "outer" boundaries.
  - Fix involves expanding original subject and protected geometries *before* subtraction: `AllowedArea = Offset(Subject, +toolRadius) - Offset(Protected, +toolRadius)`.
  - Requires updating `ResolvedPocketBand` to store original paths and refactoring `buildSurfaceCoverageRegions`.
