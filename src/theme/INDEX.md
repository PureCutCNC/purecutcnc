...
- `palette.ts` — typed 2D canvas and Three.js palette shapes plus the built-in color values consumed by the registry; also the feed-colour ramp (`FEED_COLOUR_SCALES`, `feedColourStep`, `canvasFeedColour`, `threeFeedColour`) used by the toolpath renderers (issue #498).
