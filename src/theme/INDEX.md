...
- `palette.ts` — typed 2D canvas and Three.js palette shapes plus the built-in color values consumed by the registry; also the feed-colour ramp (`feedColourScales`, `feedColourStep`, `canvasFeedColour`, `threeFeedColour`, `pocketSlotFeedPercent`), whose rungs are the engine's non-uniform feed ladder (issue #591), derived from the operation's slot feed, used by the toolpath renderers (issue #498).
