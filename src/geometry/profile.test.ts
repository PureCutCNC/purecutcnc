...
import { clonePoint, cloneProfile, transformProfile, translateProfile } from './profile'
...
function run() {
  ...
  // --- translateProfile: every segment kind moves ---
  {
    const profile = mixedProfile()
    const moved = translateProfile(profile, 10, 100)
  ...
  // --- translateProfile does not mutate its input ---
  {
    const profile = mixedProfile()
  ...
  // --- transformProfile: arbitrary point map reaches every point ---
  {
    const profile = mixedProfile()
    const scaled = transformProfile(profile, (p) => ({ x: p.x * 2, y: p.y * -1 }))
  ...
  // --- transformProfile preserves non-point profile fields ---
  {
  ...
  // --- translateProfile by zero is an identity in value terms ---
  {
    const profile = mixedProfile()
  ...
  // --- cloneProfile: deep copy, no shared point objects ---
  {
    const profile = mixedProfile()
