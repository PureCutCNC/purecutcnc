...
- `index.ts` — public API: `nest`, `createNester`, `improveNest`, the default strategies, and the types
...
- `improve.ts` — `improveNest`: keep-improving search (#862), a seeded genetic search over copy order that starts from the one-shot answer, yields every layout, and stops after `stallLimit` layouts without improvement; `isBetterNest` ranks unplaced area, then used (bounding-box) area
...
- `testLayout.ts` — layout checks shared by the engine tests: gap, sheet and obstacle clearance measured on the placed material (footprint minus holes)
- `improve.test.ts` — first step is the one-shot answer, ranking, the best never worsens and every improvement is valid, a seed reproduces the run, order fits a part the one-shot order left out, stalls end the search
