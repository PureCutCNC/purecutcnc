...
function testResolutionScalesWithShortSpan(): void {
  const large = resolveMedialResolution({ outer: rect(20, 10), islands: [] }, 'mm')
  const small = resolveMedialResolution({ outer: rect(2, 1), islands: [] }, 'mm')
...
function testLargeShapeUsesResolutionCeiling(): void {
  const result = resolveMedialResolution({ outer: rect(100, 100), islands: [] }, 'mm')
  assert(result !== null, 'expected a valid large-shape resolution')
  assert(
    approx(result.resolution, MAX_MEDIAL_RESOLUTION_MM),
    `expected ${MAX_MEDIAL_RESOLUTION_MM} mm ceiling, got ${result.resolution}`,
  )
...
function testResolutionIsUnitEquivalent(): void {
  const mm = resolveMedialResolution({ outer: rect(25.4, 12.7), islands: [] }, 'mm')
  const inch = resolveMedialResolution({ outer: rect(1, 0.5), islands: [] }, 'inch')
...
function testSampleBudgetOverridesShapeResolution(): void {
  const result = resolveMedialResolution({ outer: rect(100_000, 1), islands: [] }, 'mm')
...
try {
  ...
  testLargeShapeUsesResolutionCeiling()
