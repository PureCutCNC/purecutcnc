 ...
 * leaves a tip, and the neighbouring rings sweep a whole tool radius one
 * stepover away, so the cleanup loop looked redundant — on every corner
 * measured, the tip was reached with room to spare. Skipping it left a
 * 0.21 x 0.05in ridge standing 0.0057in proud on the fixture below, because
 * the loop had also been sweeping stock *outside* its own tip. Three other
 * pocket fixtures showed nothing. Only this one caught it.
 *
...
function testSmoothingLeavesNoStock(fixture: string): void {
  ...
  const unsmoothed = unclearedCells(project,
    { ...base, roundOutsideCorners: false, cleanWallCorners: false }, toolRadius)
  const rounded = unclearedCells(project,
    { ...base, roundOutsideCorners: true, cleanWallCorners: false }, toolRadius)
  const withWall = unclearedCells(project,
    { ...base, roundOutsideCorners: true, cleanWallCorners: true }, toolRadius)
...
try {
  // The fixture that caught the regression: a pocket whose rings pinch between
  // an elliptical island and a stepped wall.
