...
export const warningsDe: Record<keyof typeof warningsEn, string> = {
  ...
  'warnings.pocketTrochoidalWidthTooSmall': 'Die trochoidale Taschenbreite {width} liegt unter dem Minimum {minimum} (das 1,15-fache des Werkzeugdurchmessers). Verbreitern Sie den Kanal oder verwenden Sie einen kleineren Fräser.',
  'warnings.pocketTrochoidalWidthLeavesCore': 'Die trochoidale Taschenbreite {width} ist mehr als doppelt so groß wie der Werkzeugdurchmesser; der Helix-Einstieg überlappt seine eigene Mitte nicht mehr und lässt einen ungeschnittenen Kern stehen. Verengen Sie den Kanal.',
  'warnings.pocketTrochoidalStepoverHigh': 'Ein Zeilenabstand von {stepover} setzt die trochoidalen Ringe {pitch} auseinander in einem Kanal von {width}; dazwischen kann Material stehen bleiben. An einer rechtwinkligen Tasche gemessen war {limit} der höchste noch vollständig räumende Wert; bei spitzeren Ecken bleibt auch darunter Material stehen. Verringern Sie den Zeilenabstand oder prüfen Sie das Ergebnis.',
  ...
  'warnings.postToolChangesDisabled': 'Operation „{operation}" benötigt das Werkzeug „{tool}", aber Werkzeugwechsel sind deaktiviert: Das Programm schneidet weiter mit dem vorherigen Werkzeug. Aktivieren Sie die Werkzeugwechsel, oder exportieren Sie jeweils nur ein Werkzeug.',
