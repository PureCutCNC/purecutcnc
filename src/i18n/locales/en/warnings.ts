...
export const warningsEn = {
  ...
  'warnings.pocketTrochoidalWidthTooSmall': 'Trochoidal pocket cut width {width} is below the {minimum} minimum (1.15 times the tool diameter). Widen the channel or use a smaller cutter.',
  'warnings.pocketTrochoidalWidthLeavesCore': 'Trochoidal pocket cut width {width} is more than twice the tool diameter, so the helical entry no longer overlaps its own centre and leaves an uncut core for the first loops to plough through. Narrow the channel or enter elsewhere.',
  'warnings.pocketTrochoidalStepoverHigh': 'A stepover of {stepover} spaces the trochoidal rings {pitch} apart in a {width} channel, which can leave uncut material between them. Measured on a right-angled pocket, {limit} is the most that still cleared; a pocket with sharper corners leaves material below that too, because consecutive rings diverge across a corner. Lower the stepover, or check the result.',
  'warnings.pocketTrochoidalCornerReliefUnsupported': 'Corner relief ({style}) is not applied to a trochoidal pocket: the orbit never traces the ring corner the relief pass descends on, so no relief is cut. Clear the setting, or use a contour pattern if the corners must be relieved.',
  'warnings.pocketTrochoidalTightSpot': 'The gap near ({x}, {y}) is too narrow for the {width} trochoidal channel, so no pass was generated there and stock is left standing. Narrow the channel, use a smaller cutter, or clear that area with a contour pattern.',
