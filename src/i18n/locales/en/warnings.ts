...
export const warningsEn = {
  ...
  'warnings.carveTrochoidalMoveBudget': 'Trochoidal engraving exceeds the 500,000-point operation budget near ({x}, {y}).',
  'warnings.carveTrochoidalEntryBudget': 'The trochoidal engraving entry near ({x}, {y}) exceeds the entry-point budget.',
  ...
  'warnings.drillRetractBelowStockTop': 'Retract height ({requested}) is below the top of the material; raised to {clamped} so the tool does not enter the part at rapid. Retract height is stored as a distance above the material surface (issue #481).',
