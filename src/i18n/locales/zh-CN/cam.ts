...
export const camZhCN: Record<keyof typeof camEn, string> = {
  ...
  'cam.panel.add': '添加',
  'cam.plan.launch': '方案',
  'cam.plan.title': 'CAM 方案',
  'cam.plan.subtitle': '检查将添加的操作、刀具和共享设置。现有操作保持不变。',
  'cam.plan.summary': '方案摘要',
  'cam.plan.operations': '项操作',
  'cam.plan.tools': '把刀具',
  'cam.plan.existingOperations': '项现有操作',
  'cam.plan.needsReview': '项待检查',
  'cam.plan.recommendations': '建议方案',
  'cam.plan.recalculate': '刷新建议',
  'cam.plan.recalculating': '正在重新计算…',
  'cam.plan.includeOperation': '包含 {name}',
  'cam.plan.moveUp': '上移操作',
  'cam.plan.moveDown': '下移操作',
  'cam.plan.dependencyBlocked': '该移动会把操作放到其依赖项之前。',
  'cam.plan.whyOperation': '为什么建议此操作？',
  'cam.plan.mainParameters': '主要参数',
  'cam.plan.toolInsufficientCutDepth': '需要 {required} {units} 的切削深度；此刀具仅提供 {available} {units}',
  'cam.plan.toolExceedsDiameter': '最大刀具直径为 {maximum} {units}',
  'cam.plan.toolIncompatible': '不满足此操作的约束条件',
  'cam.plan.userSelectedTool': '用户已选择刀具。其保存的默认值将用于主轴转速、进给、切深和步距。',
  'cam.plan.missingTool': '创建此操作前请选择有效刀具。',
  'cam.plan.blockingErrors': '请先修复这些问题再创建加工操作',
  'cam.plan.reviewError': '查看',
  'cam.plan.useRecommendedRestTool': '使用推荐的残料加工刀具',
  'cam.plan.restStale': '源操作已更改。请刷新建议以重新计算残料加工。',
  'cam.plan.sharedSetup': '共享轮廓设置',
  'cam.plan.sharedTabs': '共享连接桥',
  'cam.plan.sharedBy': '所有匹配轮廓操作共用一次',
  'cam.plan.sharedByCount': '由 {count} 项操作共享',
  'cam.plan.reuseTabs': '这些操作复用项目中的连接桥',
  'cam.plan.createTabs': '为这些操作一次创建 {count} 个连接桥',
  'cam.plan.tabDimensions': '共享连接桥尺寸',
  'cam.plan.tabWidth': '宽度',
  'cam.plan.tabHeight': '高度',
  'cam.plan.tabTop': '顶部 Z',
  'cam.plan.tabSharedNote': '更改数值会更新此共享布局中的所有建议连接桥。',
  'cam.plan.tabsDisabledWarning': '此分离切削已禁用连接桥。加工前请确认其他工件固定方式。',
  'cam.plan.coverageReview': '需要明确检查',
  'cam.plan.excludedCoverage': '当前没有启用的建议覆盖此特征。',
  'cam.plan.acknowledge': '我了解创建的操作不会覆盖这些形状。',
  'cam.plan.noRecommendations': '未找到支持的 2.5D 建议。',
  'cam.plan.cancel': '取消',
  'cam.plan.createOperations': '创建 {count} 项操作',
  ...
  'cam.panel.exportBookletForOperation': '导出此工序的手册 (PDF)',
  'cam.panel.exportBookletForSelected': '导出所选工序的手册 (PDF)',
  'cam.panel.exportBookletFor': '导出 {name} 的手册 (PDF)',
  ...
  'cam.operation.impliedScallopAtSlope': '{angle}° 时的对应残脊：{value}',
  'cam.operation.group.identity': '标识',
  'cam.operation.group.target': '加工对象',
  'cam.operation.group.tool': '刀具',
  'cam.operation.group.depth': '深度',
  'cam.operation.group.feeds': '进给与转速',
  'cam.operation.group.strategy': '策略',
  'cam.operation.group.advanced': '高级覆盖值',
  'cam.operation.group.entry': '下刀与抬刀',
  'cam.operation.group.corners': '拐角',
  'cam.operation.group.drilling': '钻孔',
  'cam.operation.group.output': '输出',
  ...
  'cam.operation.xyLeadStrategy': 'XY 进退刀',
  'cam.operation.xyLeadNone': '直接',
  'cam.operation.xyLeadArc': '相切圆弧',
  'cam.operation.xyLeadStrategyTooltip': '刀具在何处到达最终深度，以及如何切入轮廓。“直接”会直接在起点下刀，在侧壁上会留下刀痕。“相切圆弧”会让下刀点避开该表面，再沿圆弧切入并离开，使刀具不会以下刀方式接触表面。',
  ...
  'cam.operation.engagementMode': '降低进给',
  'cam.operation.engagementModeLegacy': '仅开槽',
  'cam.operation.engagementModeEngagementFeed': '按啮合',
  ...
  'cam.paramRef.scallopHeight': '残脊高度参考',
  ...
  'cam.paramRef.xyLeadStrategy': 'XY 进退刀参考图',
  ...
  'cam.opDesc.pocket.fullDescription': '挖槽将闭合减法轮廓内部清除至固定 Z 深度。可选择清除模式：偏移（同心）、同心圆起始（先用逐渐扩大的圆清除开阔区域，再用偏移环）、平行（可配置角度的扫描线）或摆线（绕行的环，刀具啮合小）。',
  'cam.opDesc.pocket.keyPoint.0': '需要一个或多个闭合的减法轮廓',
  'cam.opDesc.pocket.keyPoint.1': '偏移、同心圆起始、平行或摆线清除模式',
  ...
  'cam.opDesc.surfaceClean.fullDescription': '表面清理加工一个或多个加法/模型特征的平坦顶面，清除位于其上方的更高加法特征周围的区域。在每个台阶高度产生一圈清理刀路——适用于修整平台、阶梯和台阶表面。模式可选偏移、同心圆起始、平行或摆线。',
  'cam.opDesc.surfaceClean.keyPoint.0': '需要一个或多个闭合的加法或模型特征',
  'cam.opDesc.surfaceClean.keyPoint.1': '在每个台阶高度清除较高特征之间的区域',
  'cam.opDesc.surfaceClean.keyPoint.2': '偏移、同心圆起始、平行或摆线清除模式',
  ...
  'cam.opDesc.drilling.fullDescription': '钻孔在每个选定圆形特征的中心钻出一个孔。在操作中选择钻孔类型和深度：简单（G81）、啄钻（G83）、暂停（G82）或断屑（G73）固定循环；螺旋，用平底立铣刀沿螺旋线加工比刀具更大的孔；或锪孔，用 V形刀加工锪孔。',
  'cam.opDesc.drilling.keyPoint.0': '需要一个或多个圆形特征',
  'cam.opDesc.drilling.keyPoint.1': '六种钻孔类型：简单（G81）、啄钻（G83）、暂停（G82）、断屑（G73）、螺旋、锪孔',
  ...
  'cam.opDesc.roughSurface.fullDescription': '曲面粗加工以恒定 Z 层（水平线风格）切片导入的 3D 模型，并用所选模式（偏移、同心圆起始、平行或摆线）清除每一层，为精加工留出径向和轴向余量。使用较大的下刀步距和横向步距以提高速度；后续使用精加工操作以保证精度。',
  'cam.opDesc.roughSurface.keyPoint.0': '需要一个导入的 3D 模型',
  'cam.opDesc.roughSurface.keyPoint.1': '水平线风格分层切片，每层以偏移、同心圆起始、平行或摆线模式清除',
  ...
  'cam.opDesc.finishSurface.fullDescription': '曲面精加工在导入的 3D 模型上生成最终表面。选择平行（可配置角度的扫描线）用于较浅的几何体，恒残留高度（沿曲面本身等距分布的走刀）用于雕刻和弧面顶板，或水平线（恒定 Z 轮廓）用于较陡的壁面。使用球头铣刀进行恒残留高度加工时，请设置残脊高度：数值越小，表面越精细，加工时间越长。',
  'cam.opDesc.finishSurface.keyPoint.0': '需要一个导入的 3D 模型',
  'cam.opDesc.finishSurface.keyPoint.1': '平行（扫描线）、恒残留高度或水平线（恒定 Z）模式',
  ...
  'cam.opDesc.finishSurfaceCleanup.fullDescription': '曲面清理在 3D 粗加工操作留下的每个台阶的最深保留 Z 处生成仅精修的壁面和底面刀路。它去重跨层的重复壁/底柱，使每处在最低有效深度仅切削一次——清理粗加工留下的阶梯而不重新粗加工。底面可用偏移、同心圆起始或平行模式清除。',
  'cam.opDesc.finishSurfaceCleanup.keyPoint.0': '需要一个导入的 3D 模型',
  'cam.opDesc.finishSurfaceCleanup.keyPoint.1': '独立的精修侧壁和精修底面开关',
  'cam.opDesc.finishSurfaceCleanup.keyPoint.2': '底面的偏移、同心圆起始或平行模式',
