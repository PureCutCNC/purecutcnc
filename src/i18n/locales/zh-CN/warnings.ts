...
export const warningsZhCN: Record<keyof typeof warningsEn, string> = {
  ...
  'warnings.edgeTrochoidalAdvanceDegenerate': '({x}, {y}) 附近的摆线每圈推进量对该刀具而言过小：每圈前进不足刀具直径的 1%，只会反复切削同一处材料而无法向前进给。请提高每圈推进量。',
  'warnings.edgeTrochoidalMoveBudget': '摆线加工所需的点数超出单个操作的上限。请提高每圈推进量、增大下刀步距以减少 Z 层数，或改用更大的刀具。',
  'warnings.edgeTrochoidalEntryBudget': '({x}, {y}) 附近的摆线入口已超出该操作剩余的点数预算。请提高每圈推进量、增大下刀步距以减少 Z 层数，或改用更大的刀具。',
  ...
  'warnings.carveTrochoidalWidthTooSmall': '摆线雕刻切削宽度必须至少为刀具直径的 1.15 倍。',
  'warnings.carveTrochoidalWidthNarrow': '摆线雕刻切削宽度较窄；建议至少为刀具直径的 1.25 倍。',
  'warnings.carveTrochoidalWidthLeavesCore': '摆线雕刻切削宽度超过刀具直径的两倍；每次螺旋下刀都会留下未切除的芯料，需由后续回转强行切除。请减小切削宽度或预钻入刀孔。',
  'warnings.carveTrochoidalAdvanceRange': '摆线雕刻每圈推进量必须大于 0 且不超过一个刀具直径。',
  'warnings.carveTrochoidalEntryStrategyUnsupported': '摆线雕刻仅支持螺旋或垂直下刀。',
  'warnings.carveTrochoidalInvalidGuide': '({x}, {y}) 附近的雕刻路径不是有效的摆线引导线。',
  'warnings.carveTrochoidalAdvanceDegenerate': '({x}, {y}) 附近的摆线雕刻每圈推进量对该刀具而言过小：每圈前进不足刀具直径的 1%，只会反复切削同一处材料而无法向前进给。请提高每圈推进量。',
  ...
  'warnings.pocketTrochoidalInvalidGuide': '摆线型腔清理需要有效的闭合引导环。',
  'warnings.pocketTrochoidalMoveBudget': '摆线型腔清理所需点数超出单个操作的上限。请提高每圈推进量、增大下刀步距以减少 Z 层数，或改用更大的刀具。',
  ...
  'warnings.pocketTrochoidalTightSpot': '({x}, {y}) 附近的通道过窄，容纳不下 {width} 的摆线通道，该处未生成刀路，会残留材料。请减小通道宽度、改用更小的刀具，或用轮廓图案清除该区域。',
  ...
  'warnings.drillSkippedUnderClamp': '{name} 位于压板“{clamp}”下方；已跳过 — 下刀无法抬升越过压板',
  ...
  'warnings.drillRetractBelowStockTop': '退刀高度（{requested}）等于或低于材料上表面；退刀平面将被限制到其上方 {clamped} 处，以免刀具以快速移动进入工件。请设置高出材料表面的正距离。',
  ...
  'warnings.clampBlockedCut': '刀具路径已避开压板“{name}” — 其下方及安全间距内的材料保留未加工。',
  ...
  'warnings.constantScallopResolutionTooCoarse': '恒残留高度间距 {spacing} 小于受限曲面网格 ({cellSize})；请限制加工区域或增大步距',
  'warnings.constantScallopEmpty': '所选模型、区域和坡度范围内没有可加工的恒残留高度曲面',
  ...
  'warnings.postToolChangesDisabled': '加工操作“{operation}”需要刀具“{tool}”，但换刀已禁用：程序会继续使用上一把刀具切削。请启用换刀，或每次只导出一把刀具。',
