...
- `NestPanel.tsx` — `NestPanelHost` (reads `pendingNest`, owns the workflow-panel position) and the panel: parts on sheet (one row per part plus an All field when several parts are selected), gap (pre-filled from the cutting tool, raise-only), rotation preset, keep originals, Nest / Stop / Discard nest, Keep improving (#862: layouts tried and the gain so far; the first better layout is one undo step and later ones amend it; any other project change stops the search), and the placed/unplaced + tabs report. Opening it on any row of an existing nest re-targets that nest: Nest replaces it in one undo step
