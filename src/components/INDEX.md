...
- `cam/` — CAM panels (tools, operations, parameters); per-parameter reference icons via `OperationParameterReference`. `ToolLibraryDialog` renders the focused library-import modal; the pure filtering/selection helpers live in `toolLibraryDialogModel.ts` with their own structural tests.
